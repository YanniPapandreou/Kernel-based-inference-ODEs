...
# function schnakenberg(x, p, t)
#   x1, x2 = x
#   w1, w2, w3, w4 = p
#   dx1 = exp(w1)*(x1^2)*x2 + exp(w2) - exp(w3)*x1
#   dx2 = -exp(w1)*(x1^2)*x2 + exp(w4)
#   return dx1, dx2
# end
...
function my_custom_train!(ps,data,d,k,N_sim,opt)
    ...
    # create the loss function for this update
    function loss()
        sim_mat = generator(ps,sim_initials)
        mmdU(data,sim_mat,k)
    end

    # compute the loss and gradient and print the loss
    training_loss, back = Zygote.pullback(loss,Flux.params(ps))
    display(training_loss)
    gs = back(one(training_loss))
...
for i in 1:max_iters
    my_custom_train!(ps,true_data,d,k,N_sim,opt)
end
